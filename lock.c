...
int modL(int x, int y) { return (x & (y - 1)); }
...
void releaseTAS(tas_t *lock) { lock->flag = 0; }

void freeTAS(tas_t *lock) { free(lock); }
...
{
  int slot = modL(__sync_fetch_and_add(&lock->tail, 1), lock->size);
  *mySlot = slot;
...
void releaseAnderson(anderson_t *lock, int mySlot)
{
  ...
  lock->flag[modL(slot + 1, lock->size)].value = true;
}
